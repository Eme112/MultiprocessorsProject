...
int validateSize(uint32_t rows, uint32_t cols, char matrix) {
    ...
    if (matrix == 'A')
        file = fopen("matrixA1048576.txt", "r");
    else
        file = fopen("matrixB1048576.txt", "r");
...
void print_table(double serial[5], double openmp[5], double cuda[5]) {
    ...
    if(promedio_serial < promedio_openmp) {
...
int main() {
    ...
    if (!readMatrixFromFile("matrixA1048576.txt", &A, rows_a, cols_a)) {
    ...
    if (!readMatrixFromFile("matrixB1048576.txt", &B, rows_b, cols_b)) {
